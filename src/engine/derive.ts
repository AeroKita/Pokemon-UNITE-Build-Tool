...
import { availableActiveBoosts, activeAttackSpeedPoints, activeStatFactors, type ActiveBoost } from "./effects";

// Whole-number stats truncate after a multiplicative buff (same rule as the
// emblem set-bonus stacking order).
const INTEGER_STATS = new Set<keyof StatBlock>(["hp", "attack", "defense", "spAttack", "spDefense", "moveSpeed"]);
...
export function deriveBuild(loadout: Loadout, inCombat = true, slotGrades?: [number, number, number]): DerivedBuild {
  ...
  if (!pokemon) {
    return { pokemon: null, base: null, effective: null, emblemLoadout, availableBoosts, buffedStats: new Set(), attackSpeed: null, oocMoveSpeed: null };
  }
  ...
  for (const [stat, factor] of Object.entries(factors) as [keyof StatBlock, number][]) {
    ...
    effective[stat] = INTEGER_STATS.has(stat) ? Math.floor(effective[stat] * factor) : effective[stat] * factor;
  ...
  const oocMoveSpeed = outOfCombatMoveSpeed(effective, equippedHeld, { inCombat: false, goalsScored: 0 });

  return { pokemon, base, effective, emblemLoadout, availableBoosts, buffedStats, attackSpeed, oocMoveSpeed };
}
