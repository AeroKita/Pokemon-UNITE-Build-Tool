...
App version shown in Settings → Updates comes from the `"version"` field in `package.json` (currently `1.3.7`), injected at build time via `vite.config.ts` (`define.__APP_VERSION__`) into `src/ui/version.ts` (`APP_VERSION`). Bump with `npm version <semver> --no-git-tag-version` (or edit `package.json` and sync `package-lock.json`).
